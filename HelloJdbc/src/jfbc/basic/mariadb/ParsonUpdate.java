...
public class ParsonUpdate {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 연결할 주소 : url, db 사용사능한 유저 :userId, dbuser인지 확인 : userPassword
			...
			String SqlUpdate = "UPDATE parson SET name = ?, age = ?, birth = ? ";
			pstmt = conn.prepareStatement(SqlUpdate);
			
			int index = 1;// sqlInsert 질의 문자열의 컬럼의 위치를 나타냄
			// 첫번쨰 컬럼인 name컬럼
			pstmt.setString(index++, "황진이");
			...
			// 세번쨰 칼럼인 brith에 값대입
			pstmt.setString(index++, "20220303");
