...
public class AAA {
    ...
    //총점계산 메소드 분리
	public static int 총점계산(int[] a, int b ) {        //메서드에 리턴값 1개가 필요 void느 리턴값 없음
		int 총점 = 0 ; //지역변수 선언 이메서드 안에서만 사용가능 //메서드() 밖에서 불어올 값들을 입력해준다.
		for (int i = 0; i < b; i++) {
			총점 =총점 +a[i];
		}
	...
	//성적 입력 메소드 분리
	public static int[] 성적입력(int 학생수) { //배출할 함수형을써주면됨 int[] int형 배열
